...
export function registerCaseTools(server: McpServer, client: InvarianceClient): void {
  server.tool(
    ...
    'Create a workflow-instance Case. For workflow work, create or find a case first, then start runs with case_id so evidence and outcomes stay linked.',
    {
      workflow_key: z
        .string()
        .max(128)
  ...
  server.tool(
    ...
    'Get normalized evidence for a case: case, runs, nodes, workflow events, actors, and outcome. Use this as the main investigation view.',
    { id: z.string().describe('Case id, e.g. "case_abc123".') },
  ...
  server.tool(
    ...
    'Attach a lightweight workflow event to a case, such as approval.requested, ticket.updated, meeting.note, or outcome.changed.',
    {
      id: z.string().describe('Case id.'),
      type: z.string().describe('Event type, e.g. "approval.requested".'),
      actor_type: z.enum(['human', 'agent', 'llm', 'service', 'integration', 'system']).optional(),
      actor_id: z.string().nullable().optional(),
      ts: z.string().optional().describe('ISO-8601 timestamp. Defaults to now.'),
      payload: z.string().optional().describe('JSON object string payload.'),
      evidence_node_ids: z.array(z.string()).optional().describe('Node ids supporting this event.'),
      evidence_refs: z.string().optional().describe('JSON array of external evidence refs.'),
    },
    async ({ id, type, actor_type, actor_id, ts, payload, evidence_node_ids, evidence_refs }) => {
  ...
  server.tool(
    'invariance_case_event_list',
    'List lightweight workflow events attached to a case.',
    { id: z.string().describe('Case id.') },
    async ({ id }) => jsonResult(await client.get(`/v1/cases/${encodeURIComponent(id)}/events`)),
  );
}
